...
let subcategories = ["egg", "meat", "sweetness", "hot drink", "cold drink", "salad", "alcohol", "muesli"];
let promotions = ["buy one get one free", "lower price", "none"];
let roles = ["admin", "costumer", "employee"];
        ...
        for (let k of keys) {
            ...
            if (tableID = "#menu") {
                ...
                if (k == "id") {
                    ...
                    let input = createAnyElement("textarea", {
                        class: "form-control",
                        rows: 4,
                        value: row[k],
                        name: k 
    ...
    for (let k of keys) {
        ...
        if (tableID = "#menu") {
            if (k == "id") {
                ...
                let options = categories;
                createNewSelectOptions(options);
                td.appendChild(input);
            } else if (k == "subcategory") {
                let options = subcategories;
                createNewSelectOptions(options);
...
function createSelectOptions(options) {
    for (let o of options) {
        let input = createAnyElement("select", {
            class: "form-select",
            value: row[k],
            name: k
        });
        option = createAnyElement("option", {
            value: o
        });
        option.innerHTML=o;
...
function createNewSelectOptions(options) {
    for (let o of options) {
        let input = createAnyElement("select", {
            class: "form-select",
            name: k
